...
The original `.docx` travels as **base64 in the request body** — the backend keeps no files on local disk. The frontend captures the upload as base64 in `UploadPage` (`fileToBase64`), holds it in `AppContext` (runtime-only — stripped before `localStorage` persistence to avoid the ~5MB quota), and sends it back at export time.

**Cross-session durability (Supabase Storage).** Holding the base64 only in the browser meant that leaving and returning to a project lost the original `.docx`, so the format-preserving export silently fell back to `reconstruct_docx` (raw, formatting gone). To fix this, when a `.docx` is uploaded **inside a project**, `UploadPage` also POSTs it to `POST /api/documents/{id}/original`, which stores the bytes in the private `document-originals` Supabase Storage bucket (`{org_id}/{document_id}.docx`) and records `documents.original_docx_path` (migration `004`). At export time, if the request body has no `original_docx_b64` but carries a `doc_id`, the export routes call `fetch_document_original_b64(doc_id, org_id)` to restore it from Storage — so format-preserving export survives across sessions and devices. All Storage access is backend-only via the service-role key (no Storage RLS policies); the bucket is private. PDF-sourced docs and pre-`004` uploads have no stored original and still fall back to `reconstruct_docx`.
...
- **`translation_memory.py`** — Supabase-backed TM lookup (org-scoped, language-aware; exact + batch). **Exact matches resolve project-first then org-wide**: a project's own approved row wins, but if none exists the lookup falls through to *any* approved row in the org for the same `(source_text, target_lang)` — including rows from sibling projects — so re-translating the same content under a new project reuses the existing translation (`tm_exact`) instead of going cold. This org-wide fallback applies **only to exact full-sentence matches** (safe to reuse); FAISS similarity and glossary stay project-isolated (`project → project_id IS NULL`) to avoid cross-project domain bleed.
...
### Supabase tables
`memberships` (RBAC), `organizations`, `invitations` (onboarding), `projects` / `project_members` / `documents` (project workspace; `documents.original_docx_path` points at the stored original in the `document-originals` Storage bucket), `translation_memory` (approved TM, carries `faiss_index`), `glossary` (`PENDING`/`VERIFIED` terms), `pipeline_events` (per-sentence analytics for Dashboard/Profile — treated as optional; absence degrades gracefully).
